...
export const fetchQuestions = async () => {
  ...
  const response_part1 = await fetch(
  ...
  const response_part2 = await fetch(
    `${GEMINI_URL}?key=` + process.env.GEMINI_API_KEY,
    {
      method: 'POST',
      headers: {
  ...
  const response_part3 = await fetch(
    `${GEMINI_URL}?key=` + process.env.GEMINI_API_KEY,
    {
      method: 'POST',
      headers: {
  ...
  const part1_string =
    responseBody_part1.candidates[0].content.parts[0].text.replace(/\n/g, '')
  ...
  // 文字列に改行が入っている事があるので削除する
  const part2_string =
    responseBody_part2.candidates[0].content.parts[0].text.replace(/\n/g, '')
  ...
  shouldSay = shouldSay.filter(
    (question) =>
      !question.includes('You should say') &&
      !question.includes('you should say') &&
      question !== '' &&
      question !== ' ',
  )
  ...
  // 文字列に改行が入っている事があるので削除する
  const part3_string =
    responseBody_part3.candidates[0].content.parts[0].text.replace(/\n/g, '')
